    ...
    def __init__(self):
        ...
        self.config = {
            ...
            '6mr_vs_1st':{
                ...
                'units_info': ['marine', 'SiegeTank'],
            },
            'bush_elsecaro':{
                'map_info':'The map is a 32*32 sized square map. The available area of x axis is from 0 to 32, and the y axis is from 0 to 32. There are two patches of tall grass located in the center of the map around coordinates (14, 17) and (19, 21). Units inside the grass can use it as concealment. The center of the map also contains elevated terrain: the high ground areas are located near the middle, splitting the top-left and bottom-right sides. These elevated cliffs can provide vision and tactical advantage for ranged units. The pathways between Player 1 and Player 2 bases are narrow and lead through the tall grass and high ground, creating natural choke points.',
                'units_info': ['marine', 'SiegeTank'],
            },
            'flat_elsecaro':{
                'map_info':'The map is a 32*32 sized square map. The available area of x axis is from 0 to 32, and the y axis is from 0 to 32. There are only flat in the map, you can move to any where you want.',
                'units_info': ['marine', 'SiegeTank'],
            },
            'corner_elsecaro':{
                'map_info':'The map is a 32*32 sized square map. The available area of x axis is from 0 to 32, and the y axis is from 0 to 32. On the left side of the map, there is a large red-colored building occupying the coordinates roughly from (0, 0) to (17, 17). This area is completely unwalkable and blocks movement.In the upper-right part of the map, there is a dense cluster of purple crystal mountains spanning approximately from (18, 18) to (30, 30). This mountain area is also unwalkable and acts as natural obstacles that units must move around.The bottom-right and top-left corners (where players start) are flat grass terrain, fully walkable. The pathways between Player 1 and Player 2 bases are relatively open but constrained by the red building on the left and the purple mountain cluster on the right, creating indirect choke points.',
                'units_info': ['marine', 'SiegeTank'],
            },
            'main_elsecaro':{
                'map_info':'The map is a 32*32 sized square map. The available area of x axis is from 0 to 32, and the y axis is from 0 to 32. The central region of the map is mostly flat grey floor [around point(15, 17) to point(19, 21)], which is fully walkable. There are multiple red-colored blocked areas along the edges and inside the map. These red zones represent unwalkable terrain such as walls and elevated structures, preventing unit movement. On the left side of the map, a large red zone blocks access to the green terrain at the corner, making it unreachable. In the middle of the map, there are two raised red platforms connected with small slopes. The slopes allow units to move between the lower and higher levels, creating terrain height differences. The raised areas can provide positional advantage for ranged units, and the slopes act as natural choke points that limit movement. The layout creates an indirect path between Player 1 and Player 2, with obstacles and slopes influencing unit positioning and strategy.',
                'units_info': ['marine', 'SiegeTank'],
            },
            'ramp_elsecaro':{
                'map_info':'The map is a 32*32 sized square map.The left side is dark ground (walkable), and the right side is grassy terrain (walkable).Red areas mark unwalkable walls and elevated platforms.There is one ramp located in the center of the map, spanning from (15, 12) to (19, 18) and the legth is about 7, connecting the left and right regions.The ramp forms the only choke point between the two bases.',
                'units_info': ['marine', 'SiegeTank'],
            },
            'corridor_elsecaro':{
                'map_info':'The map is a 32*32 sized square map. The available area of x axis is from 0 to 32, and the y axis is from 0 to 32. There are a corridor located in the center of the map around coordinates (14, 17) and (19, 21). Units inside the grass can use it as concealment. The center of the map also contains elevated terrain: the high ground areas are located near the middle, splitting the top-left and bottom-right sides. These elevated cliffs can provide vision and tactical advantage for ranged units.',
                'units_info': ['marine', 'SiegeTank'],
            },
            'test1': {
                'map_info': 'The map is a 32*32 sized square map.The left side is dark ground (walkable), and the right side is grassy terrain (walkable).Red areas mark unwalkable walls and elevated platforms.There is one ramp located in the center of the map, spanning from (15, 12) to (19, 18) and the legth is about 7, connecting the left and right regions.The ramp forms the only choke point between the two bases.The enemy units are at (27,27) point and your units are at (7,7) point initially. You can control [\'20 Marine\', \'12 Marauder\', \'4 Medivac\', \'6 SiegeTank\', \'8 Ghost\', \'8 VikingFighter\', \'8 Cyclone\', \'8 WidowMine\', \'3 Raven\', \'4 Liberator\'] units.The enemy controls [\'15 Zealot\', \'15 Stalker\', \'9 HighTemplar\', \'10 Sentry\', \'4 Colossus\', \'5 Tempest\', \'4 Disruptor\', \'4 Carrier\'] units.',
                'units_info': ['Marine', 'Marauder', 'Medivac', 'SiegeTank', 'Ghost', 'VikingFighter', 'Cyclone', 'WidowMine', 'Raven', 'Liberator', 'Zealot', 'Stalker', 'HighTemplar', 'Sentry', 'Colossus', 'Tempest', 'Disruptor', 'Carrier'],
            }
        }
